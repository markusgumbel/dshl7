...
/**  */
public enum SetOperator implements CS {

  // ACTUAL DATA

  root(null, "SetOperator", null, null),
  /** Form the convex hull with the value. The convex hull is defined over ordered domains and is the smallest contiguous superset
(interval) that of all the operand sets. */
  ConvexHull(null, "Convex hull", "H", "convex hull"),
  /** Form the set-difference with this value, i.e., exclude this element or set from the resulting set. */
  Exclude(null, "Exclude", "E", "exclude"),
  /** Form the union with this value, i.e., include this element or set in the resulting set. */
  Include(null, "Include", "I", "include"),
  /** Form the intersection with the value. */
  Intersect(null, "Intersect", "A", "intersect"),
  /** Form the periodic hull with the value. The periodic hull is defined over ordered domains and is the periodic set that contains
all contiguous supersets of pairs of intervals generated by the operand periodic intervals. */
  PeriodicHull(null, "Periodic hull", "P", "periodic hull");

  // BOILER PLATE CODE IN WHICH ONLY THE ENUM NAME MUST BE INSERTED

  public static final UID CODE_SYSTEM = ValueFactory.getInstance().UIDvalueOfLiteral("2.16.840.1.113883.5.1069");
  public static final ST  CODE_SYSTEM_NAME = ValueFactory.getInstance().STvalueOfLiteral("SetOperator");

  private final SetOperator _parent;
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private final SetOperator _parent2;
  private final SetOperator _parent3; // well, found a 3rd :(
  private EnumSet<SetOperator> _impliedConcepts = null;

  private SetOperator(SetOperator parent, String domainName, String code, String displayName) {
    this(parent, null, domainName, code, displayName);
  }

  private SetOperator(SetOperator parent, SetOperator parent2, String domainName, String code, String displayName) {
    this(parent, null, null, domainName, code, displayName);
  }

  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private SetOperator(SetOperator parent, SetOperator parent2, SetOperator parent3, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
    _parent3 = parent3;
  ...
  private EnumSet<SetOperator> getImpliedConcepts() {
  ...
  public final BL equal(ANY that) {
    ...
    if (!(that instanceof CD))
      return BLimpl.FALSE;
    else if (that instanceof SetOperator)
      return BLimpl.FALSE;
    else
  ...
  public BL implies(CD that) {
    if(this == that || this.equals(that))
      return BLimpl.TRUE;
    if(! (that instanceof SetOperator))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    final SetOperator thatSetOperator = (SetOperator)that;
    return BLimpl.valueOf(getImpliedConcepts().contains(thatSetOperator));
  }

  public SetOperator mostSpecificGeneralization(CD that) {
    if(this == that || this.equals(that))
      return this;
    if(! (that instanceof SetOperator))
      throw new UnsupportedOperationException("cannot handle argument of class " + that.getClass());
    ...
    EnumSet<SetOperator> intersection = EnumSet.copyOf(getImpliedConcepts());
    intersection.removeAll(EnumSet.complementOf(thatSetOperator.getImpliedConcepts()));
    ...
    SetOperator mostSpecificKnownGeneralization = root;
    int maxKnownSpecificity = 1;

    for(SetOperator candidate : intersection) {
  ...
  private static Map<ST,SetOperator> _codeMap = null;

  public static SetOperator valueOf(ST code) {
    // set up the _codeMap if needed for the first time
    if(_codeMap == null) {
      synchronized(SetOperator.class) { // that one time might just as well keep it thread-safe
	if(_codeMap == null) {
	  _codeMap = new HashMap();
	  for(SetOperator concept : EnumSet.allOf(SetOperator.class)) {
    ...
    if(code.nonNull().isTrue()) {
      SetOperator concept = _codeMap.get(code);
      if(concept == null)
        throw new IllegalArgumentException(("unknown code " + code + " at codeSystem = " +
          "2.16.840.1.113883.5.1069" + ", domain = " + "SetOperator"));
      else
        return concept;
    } else 
      return null;
