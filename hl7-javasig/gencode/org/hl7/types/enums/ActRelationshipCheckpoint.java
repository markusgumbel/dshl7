...
/** A code specifying when in the course of an Act a precondition for the Act is evaluated (e.g., before the Act starts for the
...
Act.)<p>
   ...
   The repeatNumber may indicate that an Act may be repeatedly executed. The checkpointCode is specifies when the precondition
   is checked and is analogous to the various conditional statements and loop constructs in programming languages "while-do"
   vs. "do-while" or "repeat-until" vs. "loop-exit".
</p>
...
<p>The checkpointCode "through" is special in that it requires the condition to hold throughout the execution of the Act, even
   throughout a single execution. As soon as the condition turns false, the Act should receive an interrupt event (see interruptibleInd)
...
public enum ActRelationshipCheckpoint implements CS {
  ...
  Beginning(null, "Beginning", "B", "beginning"),
...
false (asynchronous WHILE loop). The service must be interruptible. */
  Through(null, "Through", "T", "through");
  ...
  private final ActRelationshipCheckpoint _parent3; // well, found a 3rd :(
  ...
  // duh! what a cheap way to make a set, but really we don't expect there to be too many more than 2 parents!
  private ActRelationshipCheckpoint(ActRelationshipCheckpoint parent, ActRelationshipCheckpoint parent2, ActRelationshipCheckpoint parent3, String domainName, String code, String displayName) {
    _parent = parent;
    _parent2 = parent2;
    _parent3 = parent3;
