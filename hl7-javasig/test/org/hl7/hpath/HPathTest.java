...
public class HPathTest extends TestCase {
  ...
  public static void main(String args[]) throws Exception {
	...
	private void test(Object context, String exprString, Object... expectedResults) {
		...
		int i = 0;
		for(Object item : eval) {
			assertTrue("no extraneous results", i < expectedResults.length);
			assertEquals(item,expectedResults[i++]);
		}
		while(i < expectedResults.length)
			assertTrue("missing result["+i+"] <" + expectedResults[i++] + ">", false);
	}
	...
	// FIXME: the following have no expected results?
	...
	public void test029() {	test(_foo, "bar.foo[boo!self()].one", 1); }
}
